...
func show_box():
	...
	set_process(true)
	print("Showing box, active is true now")
	active = true
...
func hide_box():
	...
	print("hiding the box, active is false now")
	active = false
	set_process_input(false)
	set_process(false)
...
func ask_input():
	...
	if OS.get_name() == "HTML5":
		...
		if detected_mobile:
			...
			emit_signal("text_from_player", text)
			return
	if not active:
		show_box()
		active = true
...
func text_entered(text: String):
	print("About to hide the box")
	hide_box()
	print("Box hidden, emitting the signal...")
	emit_signal("text_from_player", text)
